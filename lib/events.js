...
module.exports = {
  ...
  checkAndUpdateEventStatusFromEventId: function(models, sequelize, eventId) {
    return models.events.findById(eventId).then(function(event){
      return module.exports.isEventFullyStaffed(sequelize, eventId).then(function(staffed) {
        if (!staffed && event.status === 3) {
          var v = {status: 2}
          var s = {where: {id: eventId}}
          return models.events.update(v,s).then(function(event) {
            return event;
          })
        } else if (staffed && event.status === 2) {
          var v = {status: 3}
          var s = {where: {id: eventId}}
          return models.events.update(v,s).then(function(event) {
            return event;
  ...
  bookUserOnShift: function(models, sequelize, shift, userId, favourite = false) {
    return module.exports.isFullyStaffed(shift).then(function(fullyStaffed) {
      ...
      return models.userTimesheets.scope({
          method: ['fetchOne', userId, eventShiftId]
      }).findOne().then(function (timesheet) {
          if (!timesheet) {

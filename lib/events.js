...
module.exports = {
  ...
  formatShift: function(shift, favourites, detail, stripTimesheets) {
    ...
    if (!shift.event.comments) {
      delete shift.event.comments;
    }
  ...
  formatShiftList: function(shiftList, favourites, detail, stripTimesheets, pageInfo = null) {
    if (pageInfo) {
      var {
        page,
        limit,
      ...
      return {
        ...
        shifts: shiftList.rows.slice(lowerBound, lowerBound + limit).map(function(shift) {
          return module.exports.formatShift(shift.get({ plain: true }), favourites, detail, stripTimesheets);
        }),
        page: outPage
      }
    } else {

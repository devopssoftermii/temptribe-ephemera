...
module.exports = {
  ...
  bookUserOnShift: function(models, sequelize, eventShiftId, userId, confirm = false) {
    ...
    return models.userTimesheets.findOne({
      ...
      return models.eventShifts.findById(eventShiftId);
    }).then(function(shift) {
      if (confirm) {
        return module.exports.getClashingShifts(models, sequelize, shift, userId, 'applied').then(function(shifts) {
          return Promise.all([shifts, shifts.map(function(shift) {
            return shift.getTimesheets();
          })]).catch(function(err) {
            throw err;
          });
        }).then(function([shifts, timesheetLists]) {
          return Promise.all(shifts.map(function(shift, i) {
            shift.ts = timesheetLists[i];
            return shift;
          })).catch(function(err) {
            throw err;
          });
        }).then(function(shifts) {
          return Promise.all(shifts.map(function(shift) {
            return Promise.all(shift.ts.map(function(timesheet) {
              return timesheet.update({
                status: 7
              });

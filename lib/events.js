...
module.exports = {
  ...
  applyEventStatus: function(shift) {
    return shift.getTimesheets().then(function(timesheets) {
      return timesheets.filter(function(timesheet) {
        return timesheet.status === 4;
      }).length >= shift.qty;
  ...
  bookUserOnShift: function(models, sequelize, shift, userId, favourite = false) {
    return modules.export.isFullyStaffed(shift).then(function(fullyStaffed) {
      return Promise.all([fullyStaffed, models.userTimesheets.findOne({
        where: {
          eventShiftId: shift.id,
          userId
        }
      })]);
    }).then(function([fullyStaffed, timesheet]) {
      ...
      if (confirm) {
        return module.exports.getClashingShifts(models, sequelize, shift, userId, 'applied').then(function(shifts) {
          return Promise.all([otherShifts, otherShifts.map(function(otherShift) {
            return otherShift.getTimesheets();
          })]).catch(function(err) {
            throw err;
          });
        }).then(function([otherShifts, timesheetLists]) {
          return Promise.all(otherShifts.map(function(otherShift, i) {
            otherShift.ts = timesheetLists[i];
            return otherShift;
          })).catch(function(err) {
            throw err;
          });
        }).then(function(otherShifts) {
          return Promise.all(otherShifts.map(function(otherShift) {
            return Promise.all(otherShift.ts.map(function(timesheet) {
              return timesheet.update({
                status: 7
              });
            })).then(function(timesheets) {
              return module.exports.applyEventStatus(otherShift);
            }).catch(function(err) {
              throw err;
            });
          })).catch(function(err) {
      ...
      return module.exports.applyEventStatus(shift);
    }).catch(function(err) {
      throw err;
    });
  }

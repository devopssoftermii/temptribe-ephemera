      ...
      return models.userTimesheets.findOne({
        ...
        if (timesheet.status === 4) {
          return module.exports.getClashingShifts(models, sequelize, shift, userId, 'applied').then(function(otherShifts) {
            var timesheets = otherShifts.map(function(shift) {
              return shift.timesheets[0];
            });
            return Promise.all(timesheets.map(function(timesheet) {
              ...
              return Promise.all(otherShifts.map(function(otherShift) {
                return module.exports.checkEventStatus(otherShift);
              }));
            }).catch(function(err) {
              throw err;
            });
        ...
        return module.exports.checkEventStatus(shift).then(function() {
          return status;
        });
      }).catch(function(err) {
        throw err;
